# This is code to extract the subhalo data of Type-1 and Type-2 subhalos from the simulation for TNG100-1-Dark
...
logging.basicConfig(filename='tng100dm_data.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

# sys.stdout.flush()
# sys.stderr = open("stderr.log", "w")

h = 0.6774 # Hubble constant / 100 km/s/Mpc
mdm = 8.9e6 # This is the dark matter particle mass in solar masses in the TNG100-1-Dark simulation.
...
def get_t1subhalo_data(sfid):  
    ...
    for ix in range(len(if_snap)):
        ...
        if i1*i2 == 1:
            ...
            return pos_ar[:, 0][0], pos_ar[:, 1][0], pos_ar[:, 2][0], cm_ar[:, 0][0], cm_ar[:, 1][0], cm_ar[:, 2][0], this_subh['SubhaloVmax'], this_subh['SubhaloMass']  * 1e10/h, this_subh['SubhaloLen'], sfid, matching_snap, if_tree['SubfindID'][if_snap == matching_snap], if_tree['SubhaloVmax'][if_snap == matching_snap], if_tree['SubhaloMass'][if_snap == matching_snap]*1e10/h, inf1_snap, if_tree['SubfindID'][if_snap == inf1_snap], vpeak, vel_ar[:, 0][0], vel_ar[:, 1][0], vel_ar[:, 2][0], crossing_snap, crossing_sfid
    ...
    if i2 ==1 and i1 == 0: # This would be the case wherewe have shifting to the FoF, but it was never a central. It has to be in one of the two cases.
        return pos_ar[:, 0][0], pos_ar[:, 1][0], pos_ar[:, 2][0], cm_ar[:, 0][0], cm_ar[:, 1][0], cm_ar[:, 2][0], this_subh['SubhaloVmax'], this_subh['SubhaloMass']  * 1e10/h, this_subh['SubhaloLen'], sfid, matching_snap, if_tree['SubfindID'][if_snap == matching_snap], if_tree['SubhaloVmax'][if_snap == matching_snap], if_tree['SubhaloMass'][if_snap == matching_snap]*1e10/h, -1, -1, vpeak, vel_ar[:, 0][0], vel_ar[:, 1][0], vel_ar[:, 2][0], crossing_snap
